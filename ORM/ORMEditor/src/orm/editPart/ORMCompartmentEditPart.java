...
import orm.command.ExpandStateChangeCommand;
import orm.editPart.types.ORMTypeEditPart;
...
/**
 * EditPart for Compartments
 * -(this Comment related to a older version) it is in the package orm.editPart, because when would it be elsewhere the  imageExpand and imageCollapse would not be shown when the Editor is started
...
public class ORMCompartmentEditPart extends ORMTypeEditPart {
	...
	protected IFigure createFigure() { 
		...
		if(getParent().getModel() instanceof Rolemodel || getParent().getModel() instanceof CompartmentDiagram){
	...
	/*
	 * GRAPHICAL_NODE_ROLE Policy must be installed here so that 
	 * we can make the MainContext unselectable for connection creation
	 * 
	 * */
	...
	public void refreshVisuals() {
		...
		// shows all roletypes and rolegroups names, which are in the child rolemodel of this compartment
		// the names are shown in a list and when are more then three names in the list a ... label is added
			...
			Label lab = new Label();
			 ...
			 for(AbstractRole role : children){
				 ...
				 if(role instanceof RoleType){
					 ...
					 if(sizeList<=3) rolePart.add(label);
	...
	public void buttonExpand(boolean  isEditorData){
		 ...
		 ORMGraphicalEditor editorPart = (ORMGraphicalEditor) ((DefaultEditDomain)this.getViewer().getEditDomain()).getEditorPart();
		 ...
		 Image imageExpand =  new Image(null, ImageDescriptor.createFromFile(this.getClass(),"../icons/expandArrow3.png").createImage(), SWT.IMAGE_COPY);
		 Image imageCollapse =  new Image(null, ImageDescriptor.createFromFile(this.getClass(),"../icons/collapseArrow3.png").createImage(), SWT.IMAGE_COPY);
			 if(model.isIsExpand()){	
				figure.getListAttMet().remove(figure.getAttributeFigure());
				if(!isEditorData)figure.getListAttMet().remove(figure.getMethodeFigure());
				((Label) figure.getButton().getChildren().get(0)).setIcon(imageExpand);
				 ...
				 if(!(isEditorData)){
						 parent.getEditorData().getOwnViewer().setContents(parent.getEditorData().getOwnViewer().getContents().getModel());
						
					 }
					 else{
						 parent.getEditorBeh().getOwnViewer().setContents(parent.getEditorBeh().getOwnViewer().getContents().getModel());
					 }
				 ...
				 if(!isEditorData)figure.getListAttMet().add(figure.getMethodeFigure());
	...
	public ORMCompartmentV2Figure createORMCompartmentV2Figure(Compartment model,final boolean  isEditorData){
