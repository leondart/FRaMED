...
import orm.editPolicy.ORMTypeComponentEditPolicy;
import orm.editPolicy.ORMNodeDirectEditPolicy;
import orm.editPolicy.ORMTypeXYLayoutPolicy;
...
public abstract class ORMTypeEditPart  extends AbstractGraphicalEditPart implements NodeEditPart{
	...
	@Override public void createEditPolicies() {
	 ...
	 protected void addChildVisual(EditPart childEditPart, int index) {
		 ...
		 /*
		  * when more than 3 Attributes/Methodes are added to this Type,
		  * than the label  collectAttribute/collectMethode are added and
		  * the names of the Attributes/Methodes with index >2 are collectet in the tooltip of
		  * the label collectAttribute/collectMethode
		  * 
		  * */
		 if (childEditPart.getModel() instanceof Attribute) {
			 IFigure contentPane = ((ORMTypeFigure) getFigure()).getAttributeFigure();
          if(contentPane.getChildren().size()>=3 && !(contentPane.getChildren().contains(collectAttribute))){
	       ...
	       else contentPane.add(((ORMAttributeEditPart)childEditPart).getFigure());
		 }
		 
		 if (childEditPart.getModel() instanceof Methode) {
			 IFigure  contentPane = ((ORMTypeFigure) getFigure()).getMethodeFigure();
	       
	       if(contentPane.getChildren().size()>=3 && !(contentPane.getChildren().contains(collectMethode))){
	       ...
	       else contentPane.add(((ORMMethodEditPart)childEditPart).getFigure());
		}
