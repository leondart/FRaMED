 ...
 * of all {@link Relation}s from type cyclic, total, acyclic, reflexive and irreflexive (aka
 * relationshipConstraint) and is required as all relationshipConstraints of a {@link Relation} from
 * type relationship need to be unified in a single figure. Furthermore, in case the user
 * deletes/adds a relationshipConstraint, the figure has to be updated.
 * 
...
public class ORMRelationshipConstraintEditPart extends ORMRelationEditPart {
   ...
   * cyclic, total, acyclic, reflexive and irreflexive of a single {@link Relation} from type
   * relationship.
   */
  private Label label = new Label();
  /**
   * A string, which contains the initial text of a {@link Relation} from cyclic, total, acyclic,
   * reflexive or irreflexive without unifying it with other relationshipConstraints.
   */
  ...
  /**
   * A setter for the initial text of a {@link Relation} from cyclic, total, acyclic, reflexive or
   ...
   * total, acyclic, reflexive and irreflexive(aka relationshipConstraint) figures" of a single
   * {@link Relation} from type relationship" and sets the visibility of the relationshipConstraint
   * figure. The unifying of the relationshipConstraint figures is done through combining the
   * text/name of all the relationshipConstraints in the {@link Label} of the relationshipConstraint
   * figure of the first relationshipConstraint, which was created for the relationship, and through
   * setting the visibilities of the other relationshipConstraint figures on false.
   * 
   */
  /**
   * 04.04.2016 Duc Dung Dam Every constraint has an editpart and every editparts expect an
   * connection figure as a return. Because of changing the representation of constraints
   * (constraints connection won't show as a separate line, but the relationship connection will be
   * modified ) these connection figures needs to be set invisible.
   */
