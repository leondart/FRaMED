...
/**
 * This {@link EditPart} is the controller for a {@link Shape} from type
 * compartmenttype.
 * 
...
public class ORMCompartmentEditPart extends ORMShapeWithSegmentEditPart {
	 ...
	 * rolegroup that the {@link Model} of this compartment contains are listed.
	 * A global variable is helpful, because this {@link PartFigure} is used in
	 * the methods refreshVisual() and addChildVisual().
	 */
	private PartFigure rolePart = null;
	/**
	 * The {@link ORMModelEditPart} of the {@link Model}, which is owned by this
	 * compartment. A global variable for this is nessecary, because the rmed is
	 * set addChildVisual() method in this class and than used in buttonExpand()
	...
	/**
	 * {@inheritDoc} The refreshVisuals of this {@link EditPart} the names of
	 * all roleTypes and roleGroups in the child {@link Model} are added to the
	 * rolePart and the method {@link ORMSuperShapeEditPart#refreshVisuals()} is
	 * called.
	 * 
	 */
	@Override
	...
	/**
	 * The first thing, which is done in this method is that a {@link Label}
	 * with the text ROLES is added to the rolesPart. After that all names of
	 * all {@link Shape}s from type roletype and rolegroup, which are in the
	 * child {@link Model} of this compartmenttype, are added to the rolePart.
	 * When are more then three names in the rolePart list a ... label is added
	 * in the tooltip of the ... label the names of the remaining roletypes and
	 * rolegroups are shown.
	 * */
		...
		for (ModelElement role : model.getElements()) {
			if (role instanceof Shape) {
				...
				if (shape.getType().equals(Type.ROLE_TYPE)
						|| shape.getType().equals(Type.ROLE_GROUP)) {
	...
	/**
	 * When the user has stepped in a compartmenttype than the attribute and
	 * operation segment should be shown in a structure, which can expanded and
	 * collapsed through a button. The functionality of the button also
	 * expanding and the collapsing are done in this method.
	 * */
	private void buttonExpand(final boolean isEditorData) {

		// get ormgraphicaleditor which owns the compartment to get parent
		// multipageeditor
		final ORMGraphicalEditor editorPart = (ORMGraphicalEditor) ((DefaultEditDomain) this
				.getViewer().getEditDomain()).getEditorPart();
		// get parent multipageeditor to synchronize the behavouir and data
		// ORMGraphicalEditors
		final ORMMultiPageEditor parent = (ORMMultiPageEditor) editorPart
				.getParentEditor();
		...
		final Image imageExpand = new Image(null, Activator
				.imageDescriptorFromPlugin(Activator.PLUGIN_ID,
						"icons/expandArrow3.png").createImage(), SWT.IMAGE_COPY);

		final Image imageCollapse = new Image(null, Activator
				.imageDescriptorFromPlugin(Activator.PLUGIN_ID,
