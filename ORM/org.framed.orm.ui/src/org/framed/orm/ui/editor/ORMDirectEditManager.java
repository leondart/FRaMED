...
public class ORMDirectEditManager extends DirectEditManager {
  ...
  /**
   * Validates the entered string for valid operation, attribute, parameter or cardinality
   * expressions
   */
  @Override
  protected void handleValueChanged() {
    ...
    if (checkForNameInput()) {
      ...
      // is no digit (for
      // names)
      String qualifiedIdentifier = "(" + identifier + "\\.)*" + identifier; // full classes: (N.)*N
                                                                            // (for types)
      ...
      String operation =
          identifier + "\\((" + parameter + "(," + parameter + ")*)?\\):" + qualifiedIdentifier;
      ...
      if (m_operation.matches() || m_attribute.matches() || m_parameter.matches()
          || m_cardinali.matches() || m_rolegroup.matches()) {
        matching = true;
  ...
  private boolean checkForNameInput() {
    if (getEditPart().getModel() instanceof Shape) {
      if (!((Shape) getEditPart().getModel()).getType().equals(Type.ROLE_GROUP)) {
        return false;
      }
    } else {
      if (getEditPart().getModel() instanceof Relation) {
        if (((Relation) getEditPart().getModel()).getType().equals(Type.RELATIONSHIP)) {
          return false;
        }
      }
    }
