...
public class TestEditor2 extends EditorPart{
  ...
  protected void setInput(IEditorInput input) {
    ...
    //hier nicht, da propertyChange erst später in super.setinput aufgerufen wird
    //updateTree();
  }
...
//  }
  ...
  protected boolean errorMessage(Tree tree) {
    if (ormMultiPageEditor.getConfiguration() == null) {
        if (ormMultiPageEditor.getFeatureModelFile() == null) {
            displayError(tree, THERE_IS_NO_FEATURE_MODEL_CORRESPONDING_TO_THIS_CONFIGURATION_COMMA__REOPEN_THE_EDITOR_AND_SELECT_ONE_);
        } else if (!ormMultiPageEditor.getFeatureModelFile().exists()) {
            displayError(tree, THE_GIVEN_FEATURE_MODEL + ormMultiPageEditor.getFeatureModelFile().getPath() + DOES_NOT_EXIST_);
        } else {
            displayError(tree, AN_UNKNOWN_ERROR_OCCURRED_);
        }
        return false;
    } else {
        final FeatureModelAnalyzer analyzer = ormMultiPageEditor.getConfiguration().getFeatureModel().getAnalyser();
        try {
            if (!analyzer.isValid()) {
                displayError(
                        tree,
  ...
  protected void computeTree(boolean redundantManual) {
    ...
    updateJob.addJobFinishedListener(new JobFinishListener() {
        ...
        public void jobFinished(IJob finishedJob, boolean success) {
            if (success) {
                ...
                ormMultiPageEditor.getConfigJobManager().startJob(computeColoring(currentDisplay));
            }
        }
    });
    ...
    walkTree(new IBinaryFunction<TreeItem, SelectableFeature, Void>() {
        ...
        @Override
        public Void invoke(Void t) {
          ormMultiPageEditor.getConfigJobManager().startJob(updateJob);
            return null;
        }
    });
}
  ...
  protected void updateInfoLabel(final Display display) {
    ...
    final boolean valid = ormMultiPageEditor.getConfiguration().isValid();

    final IConfigJob<Long> job = ormMultiPageEditor.getConfiguration().getPropagator().getJobWrapper().number(250);
    job.addJobFinishedListener(new JobFinishListener() {
        ...
        public void jobFinished(IJob finishedJob, boolean success) {
            ...
            if (number == 0 && !ormMultiPageEditor.isAutoSelectFeatures()) {
    ...
    ormMultiPageEditor.getConfigJobManager().startJob(job);
}
  ...
  protected IConfigJob<?> computeFeatures(final boolean redundantManual, final Display currentDisplay) {
    ...
    final IConfigJob<?> job = ormMultiPageEditor.getConfiguration().getPropagator().getJobWrapper()
  ...
  protected IConfigJob<?> computeColoring(final Display currentDisplay) {
      if (!ormMultiPageEditor.isAutoSelectFeatures() || ormMultiPageEditor.getConfiguration().isValid()) {
          return null;
      }
      final List<SelectableFeature> featureList = ormMultiPageEditor.getConfiguration().getManualFeatures();
      final IConfigJob<?> job = ormMultiPageEditor.getConfiguration().getPropagator().getJobWrapper().leadToValidConfiguration(featureList);
      job.setIntermediateFunction(new IFunction<Object, Void>() {
          @Override
          public Void invoke(Object t) {
              if (t instanceof SelectableFeature) {
