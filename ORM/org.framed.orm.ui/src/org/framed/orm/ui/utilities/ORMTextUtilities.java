...
public class ORMTextUtilities {
  ...
  private static class TextCalc extends TextFlow {
    ...
    /**
     * Calculate the width of a given text for the given font f; If f == null the system font will
     * be used
     */
    int textWidth(String text, Font f) {
      return (f == null) ? getTextUtilities().getStringExtents(text,
          Display.getCurrent().getSystemFont()).width() : getTextUtilities().getStringExtents(text,
          f).width();
    }

    /**
     * Calculate the height of a given text for the given font f; If f == null the system font will
     * be used
     */
    int textHeight(String text, Font f) {
      return (f == null) ? getTextUtilities().getStringExtents(text,
          Display.getCurrent().getSystemFont()).height() : getTextUtilities().getStringExtents(
          text, f).height();
    }
  }
